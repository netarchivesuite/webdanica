...
if [ ! -f "$DATABASEJAR" ]; then
 echo ERROR: The database jarfile \"$DATABASEJAR\" does not exist. Please correct the path in this script \($ME\)
 exit
fi

NAS_COMMON_JAR=lib/common-core-$NAS_VERSION.jar
if [ ! -f "$NAS_COMMON_JAR" ]; then
 echo ERROR: The Netarchivesuite common-core jarfile \"$NAS_COMMON_JAR\" does not exist. Please correct the path in this script \($ME\)
 exit
fi
NAS_HARVESTER_JAR=lib/harvester-core-$NAS_VERSION.jar
if [ ! -f "$NAS_HARVESTER_JAR" ]; then
 echo ERROR: The Netarchivesuite harvester-core \"$NAS_HARVESTER_JAR\" does not exist. Please correct the path in this script \($ME\)
 exit
fi
