...
SCRIPTPATH=/home/test/templates/criteriaRun-combinedCombo-seq.pig

DATESTAMP=`/bin/date '+%d-%m-%Y-%s'`

if [ ! -d "$SEQ_BASEDIR" ]; then
 echo $SEQ_BASEDIR does not exist. The path must be wrong!
 exit	
fi
...
if [ ! -f "$FILE" ]; then
 echo seqfile $FILE does not exist. The parsed-text computation must have gone wrong
 exit
fi
