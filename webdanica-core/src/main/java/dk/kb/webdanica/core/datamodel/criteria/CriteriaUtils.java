...
public class CriteriaUtils {
    ...
    public static String findC9eval(String c9b, String c9e) {
        ...
        if (c9b != null && (!c9b.isEmpty() && !c9b.startsWith("0"))) {
            ...
            List<String> words2 = Arrays
                    .asList(Words.virksomheder_lowercased_2_words_Nov2);
    ...
    public static String findC8cval(String c8a, String c8c) {
        ...
        if (c8a != null && (!c8a.isEmpty() && !c8a.startsWith("0"))) {
            ...
            List<String> words2 = Arrays
                    .asList(Words.foreninger_lowercased_2_words_Nov2);
    ...
    public static String findC10cval(String c10c) {
        ...
        if (c10c != null && (!c10c.isEmpty())) {
            if (!c10c.startsWith("0")) {
                Set<String> tokens = TextUtils.tokenizeText(c10c.substring(1)
                        .trim());
    ...
    public static String find8bVal(String url) {
        ...
        String val = (foundMatches.size() > 0 ? (foundMatches.size() + " " + TextUtils
                .conjoin("#", foundMatches)) : "0");
        return val;
    }

    /*
     * public static String findNew3Val(String previous) { String val = ""; if
     * (previous.startsWith("0")) { val = "0"; } else { Set<String> foundMatches
     * = computeNewC3x(previous); val = (foundMatches.size() > 0 ?
    ...
    public static String findNew3ValToken(String previous) {
        ...
        if (previous.startsWith("0")) {
            ...
            for (String word : tokens) {
                if (TextUtils.findMatches(word,
                        FrequentWords.frequentwordsWithDanishLettersCodedNew)
                        .size() > 0) {
                    foundMatches.add(word);
                }
            }
            val = (foundMatches.size() > 0 ? (foundMatches.size() + " " + TextUtils
                    .conjoin("#", foundMatches)) : "0");
        }
        return val;
    }
    ...
    public static Set<String> computeC8b(String text) {
        return TextUtils.SearchPattern(text, Words.foreninger_lowercased);
    }

    /*
     * public static Set<String> computeNewC3x(String text) { return
     * TextUtils.SearchPattern(text,
    ...
    public static File checkDir(String dirname) {
        ...
        if (!statDir.isDirectory()) {
            System.err.println("ERROR: Cannot find dir'"
                    + statDir.getAbsolutePath() + "' as a proper directory");
            System.exit(1);
        }
        return statDir;
    }
    ...
    public static String getStringSequence(Collection<String> strSet,
            String delim) {
        String s = "";
    ...
    public static String getIntegerSequence(Collection<Integer> intSet,
            String delim) {
        String s = "";
        for (int i : intSet) {
    ...
    public static String getBooleanSequence(Collection<Boolean> boolSet,
            String delim) {
        String s = "";
    ...
    /**
     * @param text
     *            A given sample of text to convert to base64
     * @return the given text in Base64 encoding after first retrieving the
    ...
    public static String findCharsetFromName(String name) {
        ...
        if (index == -1) {
            System.err
                    .println("Unable to deduce the charset from the filename '"
                            + name + "': No suffix found");
            return null;
        }
        ...
        if (index == -1) {
            System.err
                    .println("Unable to deduce the charset from the filenamepart '"
                            + udenSuffix + "': missing correct separator (_)");
            return null;
        }
