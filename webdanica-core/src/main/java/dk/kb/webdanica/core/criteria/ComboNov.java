 ...
 * 
 */
public class ComboNov extends EvalFunc<String> {
    @Override
    public String exec(Tuple input) throws IOException {
        ...
        if (Cext1 > 0) {  
            ...
            if (percent.intValue() < 200) {
                ...
                result += ", C2b: " + (C2b? "y": "n");
	            Set<String> C3gmatches = C3g_fom_b_Nov.computeC3g(text);
	            result += addResultForCriterie("C3g", C3gmatches);
	            Set<String> C6dmatches = C6d_from_a_Nov.ComputeC6d(text);
	            result += addResultForCriterie("C6d", C6dmatches);
                Set<String> C7gmatches = C7g_from_a_Nov.computeC7g(text);
                result += addResultForCriterie("C7g", C7gmatches);
                Set<String> C7hmatches = C7h_from_e_Nov.computeC7h(text);
                result += addResultForCriterie("C7h", C7hmatches);
                Set<String> C8cmatches = C8c_from_a_Nov.computeC8c(text);
                result += addResultForCriterie("C8c", C8cmatches);
                Set<String> C9ematches = C9e_from_b_Nov.computeC9e(text);
                result += addResultForCriterie("C9e", C9ematches);
                boolean C9f = C9f_from_d_Nov.computeC9f(text);
                result += ", C9f: " + (C9f? "y": "n");
                Set<String> C10cmatches = C10c_from_b_Nov.computeC10c(text);
