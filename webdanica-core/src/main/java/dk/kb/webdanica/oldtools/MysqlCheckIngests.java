...
public class MysqlCheckIngests {
    /**
     * @param args <file-to-check> startInterval(int) endInterval(int) isHadoopUpdate=true|false
    ...
    public static void main(String[] args) throws IOException, FileNotFoundException, InstantiationException, IllegalAccessException, ClassNotFoundException {
        /* Read and check file with contents on form:
         * 1. line:
         *  'Running WebdanicaJobs-12-08-2014 with jdbcUser=test IADATA=true checkDoublets=true listIgnored=true on files in folder wd002-d2-m1_part-m-00'
         * followed by repeated blocks of  
         *  '--- Processing file:  m1-d2_part-m-00/part-m-00842 '
         ...
         *  ...
         * If errors have occurred then a block will only have 
         *  '--- Processing file:  wd002-d2-m1_part-m-00/part-m-00842 '
         * Skipping is when url's are already processed - for  listIgnored=true means that urls and dates for ignored are listed on form:
         *  ' - <url and date>'
         */

    	String errArgTxt = "Proper args: <file-to-check> startInterval(int) endInterval(int) isHadoopUpdate=true|false";
    	////////////////////////////////////////////////////////
        ...
        MysqlX.CheckResult chkRes = checkIngestResFile(checkFile, startInterval, endInterval, isHadoopUpdate);
        ...
        System.out.println("of which are Ignored (poss. 2. ingest): " + chkRes.ignoredOrUpdateCount );
    ...
    public static MysqlX.CheckResult checkIngestResFile(File checkFile, int startInterval, int endInterval, boolean hadoop)  throws IOException {
    	if (!checkFile.exists()) {
        ...
        ////////////////////////////////////////////////////////
        // Reconize (ingest) blocks of :
        // '--- Processing file:  wd002-d2-m1_part-m-00/part-m-00842 '
        // 'Processed 33479 lines'  // inserted = <Processed lines> - <Skipped lines> - <Ignored lines> 
        // 'Skipped 0 lines'        // lines that could not be prepared or inserted in database
        // 'Ignored 0 lines'        // lines that were already in database
        //
        // Reconize (hadoop update) blocks of :
        // '--- Processing file:  /home/test/resultater/m1-d2-p1-15-v5/part-m-00000'
        // 'Processed 33479 lines'  // updated = <Processed lines> - <Skipped lines> - <Ignored lines> 
        // 'Skipped 0 lines'        // lines that could not be prepared or inserted in database
        // 'Updated 0 lines'        // lines that updated 
        //
        ...
        if ((nextLine = fr.readLine()) == null ) {
	        ...
	        if (hadoop) {
	        	...
	        	if (cr.ignoredOrUpdateCount > 0 ) {
	        		cr.warningSet.add("WARNING: Ignored lines in ingest (can be ingested before)");
	        	}
	        }
        }
	    return cr;
